...
ALLURE_IDS_STAGE=$(./allurectl launch list -p "${ALLURE_PROJECT_ID}" --no-header | grep "${CI_PIPELINE_ID}" | cut -d' ' -f1 || true)
ALLURE_ID_STAGE=${ALLURE_IDS_STAGE:7:3}
export ALLURE_ID_STAGE
...
<i>https://testops.ftr.group/launch/${ALLURE_ID_STAGE}</i>
EOT
